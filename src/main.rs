...
impl TypeMapKey for LlmInterface {
    type Value = Arc<dyn LLMProvider>;
}

struct ThinkRemoverRegex;

impl TypeMapKey for ThinkRemoverRegex {
...
async fn ping(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    let messages = vec![
        ChatMessage {
            role: ChatRole::User,
            content: "Hello, I just said 'ping', how do you respond? Try to sound kinda goofy"
                .into(),
        },
        // ChatMessage {
    ...
    let llm = {
        ...
        data_read
            .get::<LlmInterface>()
            .expect("Expected LlmInterface in TypeMap.")
            .clone()
    };
    ...
    match llm.chat(&messages).await {
        Ok(text) => response = String::from(text),
        Err(e) => eprintln!("Chat error: {}", e),
    }

    msg.reply(ctx, re.replace_all(&response, "")).await?;
...
async fn command_usage(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if amount == 0 {
        msg.reply(
            ctx,
            format!("The command `{command_name}` has not yet been used."),
        )
        .await?;
    } else {
        msg.reply(
...
async fn owo_count(ctx: &Context, msg: &Message) -> CommandResult {
    ...
    if count == 1 {
        ...
        msg.reply(ctx, format!("OWO Has been said {count} times!"))
            .await?;
    }
